...
import { ArrowRight, Waves, Sun, Fish, Shield, ChevronLeft, ChevronRight } from 'lucide-react';
        ...
        {['Towns', 'Beaches', 'Things to Do', 'Food & Drink'].map((item) => (
          <Link 
            ...
            className={`text-base font-medium mx-4 transition-colors duration-300
              ${item === 'Beaches' ? 'text-blue-600 hover:text-blue-800' : 'text-gray-600 hover:text-blue-600'}`}
          >
            {item}
          </Link>
        ))}
  ...
  const BeachSkeleton = () => (
    <div className="bg-white rounded-lg overflow-hidden shadow-lg">
      ...
      <div className="p-6">
        ...
        <div className="flex flex-wrap mb-4">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-6 w-16 bg-gray-300 mr-2 mb-2 rounded animate-pulse"></div>
          ))}
        </div>
    ...
    useEffect(() => {
      const fetchBeaches = async () => {
        try {
          ...
          const transformedData = await Promise.all(data.map(async (beach) => {
        ...
        <div className="bg-gradient-to-b from-blue-50 to-white min-h-screen">
          ...
          <div className="max-w-7xl mx-auto px-4 py-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {isLoading
                ? Array.from({ length: 6 }).map((_, index) => <BeachSkeleton key={index} />)
